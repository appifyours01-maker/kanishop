...
class ShiprocketService {
  ...
  Future<Map<String, dynamic>> _proxyRequest(
    ...
    try {
      ...
      final url = Uri.parse(
        '${ShiprocketConfig.baseUrl}/api/shiprocket$endpoint',
      );
  ...
  Map<String, dynamic> _convertResponseToStrings(Map<String, dynamic> data) {
    ...
    data.forEach((key, value) {
      if (value is int || value is double) {
        ...
        converted[key] = _convertResponseToStrings(
  ...
  Future<PincodeResult> checkPincode(String pincode) async {
    ...
    try {
      final result = await _proxyRequest('POST', '/check-pincode', {
        'pincode': pincode,
      });
  ...
  Future<List<CourierOption>> getCourierRates({
    ...
    debugPrint(
      '🚚 SR: Fetching courier rates for pincode: $pincode, order value: $orderValue',
    );
  ...
  Future<OrderResult> placeOrder({
    ...
    debugPrint(
      '📦 SR: Placing order for ${address.fullName} at ${address.pincode}',
    );
    ...
    for (var idx = 0; idx < items.length; idx++) {
      ...
      debugPrint(
        '  📦 SR: Item $idx: $itemName (SKU: $sku, Qty: $qty, Price: $price)',
      );
    }
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        return OrderResult(
          ...
          message:
              result['message']?.toString() ?? 'Order placed on Shiprocket ✅',
        );
      } else {
...
class _DCPState extends State<DeliveryCheckoutPage> {
  ...
  Future<void> _placeOrder() async {
    ...
    final orderItems = _items
        .map(
          (i) => {
            ...
            'price': (i.effectivePrice is num
                ? (i.effectivePrice as num).toDouble()
                : 0.0),
    ...
    if (result.success) {
      ...
      try {
        widget.cartManager.removePurchasedItems(orderItems);
      } catch (_) {}
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: _tf(_stCtrl, 'State *', Icons.map, TextInputType.text),
                ),
              ],
            ),
          ],
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: List.generate(6, (i) {
                ...
                final sel =
                    _selDate.day == dt.day && _selDate.month == dt.month;
        ...
        _card(
          ...
          _loadingC
              ...
              ? Container(
                  padding: const EdgeInsets.all(16),
                  child: const Text(
                    'No couriers available. Check pincode.',
                    style: TextStyle(color: Colors.grey),
                  ),
                )
              : Column(
      ...
      _card(
        ...
        Column(
          children: [
            ...
            _brow(
              'Shipping (${_selCourier?.courierName ?? ''})',
            ...
            if (_selCourier != null)
              _brow(
                'Est. Delivery',
                '${_selCourier!.estimatedDays} days',
                valueColor: Colors.green,
              ),
            if (_selSlot != null)
              _brow(
                'Time Slot',
                '${_selSlot!.label} • ${_selSlot!.timeRange}',
        ...
        Column(
          children: [
            ...
            _payOpt(
              'cod',
              'Cash on Delivery',
              _selCourier?.codAvailable == true
                  ? 'Pay when order arrives'
                  : 'Not available for selected courier',
              Icons.money,
              Colors.green,
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '${_a1Ctrl.text}, ${_cityCtrl.text}, ${_stCtrl.text} – ${_pinCtrl.text}',
  ...
  Widget _orderSummary() => Column(
    children: [
        ...
        (i) => Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              Text(
                '${_currency}${((i.effectivePrice as num) * (i.quantity as num)).toStringAsFixed(2)}',
                style: const TextStyle(
                  fontSize: 13,
      ...
      if (_discountAmount > 0)
        _brow(
          'Discount',
          '-${_currency}${_discountAmount.toStringAsFixed(2)}',
          valueColor: Colors.green,
        ),
      if (_gstAmount > 0)
        _brow('GST', '${_currency}${_gstAmount.toStringAsFixed(2)}'),
      if (_shipping > 0)
        _brow('Shipping', '${_currency}${_shipping.toStringAsFixed(2)}'),
      const Divider(height: 16),
      Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          ...
          Text(
            '${_currency}${_grand.toStringAsFixed(2)}',
            style: const TextStyle(
              fontWeight: FontWeight.bold,
              fontSize: 14,
              color: Colors.blue,
  ...
  Widget _brow(String l, String v, {bool bold = false, Color? valueColor}) =>
      Padding(
          ...
          children: [
            ...
            Text(
              ...
              style: TextStyle(
                ...
                color:
                    valueColor ?? (bold ? Colors.black : Colors.grey.shade800),
              ),
            ),
          ],
        ),
